...
```properties
switcher.context -> Path of the feature class that extends SwitcherContext
...
```properties
switcher.url -> Swither-API endpoint (default value uses Switcher API cloud endpoint)
...
switcher.retry -> Time given to the module to re-establish connectivity with the API 
